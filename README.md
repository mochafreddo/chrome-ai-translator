...
- Inline translation serializes one semantic paragraph, heading, list item, or table cell with protected tokens for inline elements. A block and a batch are each capped at 12,000 serialized characters; one page visit is capped at 150,000 of serialized record cost, which is not a count of the page's own characters. That cap covers the second request a repair sends, survives **Original text** and stop-then-restart, and is cleared only by reloading the page — see [ADR-0007](docs/adr/0007-charge-the-session-budget-in-actual-record-cost.md). Output caps scale from 4,096 to 16,000 tokens. Oversized or malformed blocks remain unchanged instead of falling back to fragment translation.
